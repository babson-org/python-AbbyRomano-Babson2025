...
def draw_diamond():
    '''
    what happens if text, even, negative or very large number is entered?
    get height in while True block and check that it is odd and not too big.
...
def text_analysis():
    ...
    '''
    the above is fine but you could have done it  in one loop:
...
def caesar_cipher(text, shift, choice): 
    ...
    for char in text:
        if char.isalpha():
            ...
            ord(char) - start -> zero bases the ascii table for letters
            + actual shift gives us the shifted value assumming 'a' or 'A' is 0
            % 26 makes sure we get a value between 1 and 25
            + start gives us back the ascii ord() for shifted char
